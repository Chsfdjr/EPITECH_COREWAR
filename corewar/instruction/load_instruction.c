...
int get_ld_value(info_t *info, int champ_index, int offset)
{
...
info, CHAMP->pc + offset + 1, 1, IND_SIZE) % IDX_MOD, 0, REG_SIZE);
    return 0;
}
...
int ld_instruction(info_t *info, int champ_index)
{
    ...
    if (CHAMP->nb_cycle_here == 0) {
        ...
        get_register_index(info,CHAMP->pc, champ_index, 2) >= 0) {
            REGISTER_X(2) = get_ld_value(info, champ_index, 1);
            CHAMP->carry = 1;
        } else
            CHAMP->carry = 0;
        free(params);
